...
const Card = ({ isUser, isFriend }) => (
    <div className={`grid grid-cols-[auto_1fr_minmax(50px,_auto)] gap-2 items-center p-4`}>
        ...
        {isUser
            ? (
                <div className="flex gap-2">
                    <Div width="2rem" height="2rem" borderRadius="12px" className="ml-auto" />
...
const Cards = ({ count, isUser, isFriend }) => (
    Array(count).fill(0).map((item, idx) => (
        <Card key={idx} isUser={isUser} isFriend={isFriend} />
    ))
)
