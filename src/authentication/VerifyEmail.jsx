...
const VerifyEmail = ({ user, selectedTap, setSelectedTap }) => {
    ...
    const [timer, setTimer] = useState(() => {
        ...
        if(sessionTimer && !sessionTimer.isFinished && verify?.email === user.email) {
            return sessionTimer
        }
    ...
    useEffect(() => {
        timer.isFinished && sessionStorage.setItem("email-verify", JSON.stringify({isSend: false, email: user.email}))
